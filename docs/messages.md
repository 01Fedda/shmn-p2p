# Messages
The shmn protocol specifies a set of [messages](https://en.bitcoin.it/wiki/Protocol_specification) that can be sent from peer to peer. `shmn-p2p` provides support for some of these messages.
...
As well as sending "tx" and "block" messages with SHMN-js instances:
...
## List of Messages
### Version
The version message (`ver`) is used on connection creation, to advertise the type of node. The remote node will respond with its version, and no communication is possible until both peers have exchanged their versions. By default, shmn-js advertises itself as named `shmn-js` with the current version of the `shmn-p2p` package.
